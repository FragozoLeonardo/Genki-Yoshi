    ...
    (1..rows).each do |row|
      ...
      (1..cols).each do |col|
        ...
        if is_youon_start && is_youon_pair
          ...
          draw_char(pdf, current_char, top_left, cell_size, start_col) if input_index < characters.length
        end
      end
...
def generate_genkoyoshi_pdf(file_name, hiragana, katakana, kanji, convert_to_image: true)
  pdf_path = file_name
  ...
  Prawn::Document.generate(pdf_path, page_size: 'A4', margin: [25, 25, 25, 25]) do |pdf|
    ...
    draw_grid_page(pdf, hiragana, rows, cols, cell_size)
    pdf.start_new_page
    draw_grid_page(pdf, katakana, rows, cols, cell_size)
    pdf.start_new_page
    draw_grid_page(pdf, kanji, rows, cols, cell_size)
  end
